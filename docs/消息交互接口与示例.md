...
- **请求**：客户端发布到 **`mchat/msg/req/{client_id}/{seq_id}`**。`client_id` 为当前连接身份，`seq_id` 为本次请求唯一序号（如 UUID）。
- **响应**：服务端发布到 **`mchat/msg/resp/{client_id}/{seq_id}`**。客户端须**先订阅** `mchat/msg/resp/{client_id}/+` 再发请求。
- **Payload**：均为 JSON，UTF-8 编码。请求中必须含 **`action`** 与 **`seq_id`**；响应中统一含 **`seq_id`**、**`code`**（0 成功，非 0 失败）、**`message`**、**`data`**（成功时可为对象或空）。
...
以下每个接口均给出 **Topic** 与 **Payload** 请求/响应（或投递）示例。
