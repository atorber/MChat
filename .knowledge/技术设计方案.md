...
- **请求**：所有操作请求发布到 `mchat/msg/req/{client_id}/{seq_id}`，payload 含 action、seq_id 及业务参数。seq_id 客户端生成（UUID 或雪花 ID），保证未完成请求内唯一。
...
- 客户端维护 in-flight 映射：seq_id → { resolve, reject, timeout }。收到 `mchat/msg/resp/{client_id}/{seq_id}` 时解析 payload，用 seq_id 找到 Promise 并 resolve(code, message, data)。
