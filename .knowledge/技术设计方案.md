...
服务端使用**统一配置文件**管理 MQTT 连接信息、MySQL（关系库）连接信息与对象存储鉴权信息，便于部署与环境切换（开发/测试/生产）。推荐单文件（如 `config.yaml` 或 `config.json`），通过环境变量指定路径或环境名；敏感字段支持环境变量占位或独立 secrets 文件（不纳入版本库）。

| 配置项 | 说明 | 示例/用途 |
|--------|------|-----------|
| **MQTT 连接** | 服务端连接 Broker 所用参数 | `broker.host`、`broker.port`、`broker.useTls`、`broker.clientId`（或带实例 ID）、`broker.username`/`broker.password` 或证书路径；可选 `broker.shareSubscription`（共享订阅组名） |
| **MySQL 连接** | 关系库（组织、员工、群组、会话映射、审计等）连接参数 | `mysql.host`、`mysql.port`、`mysql.user`、`mysql.password`、`mysql.database`；可选 `mysql.poolSize`、`mysql.connectTimeout` 等连接池参数 |
| **对象存储鉴权** | 服务端生成临时凭证或预签名 URL 时使用的后端账号 | `storage.endpoint`、`storage.region`、`storage.bucket`、`storage.accessKey`、`storage.secretKey`；或 STS 角色、IAM 策略等（依实现而定）。用于 config.storage、file.upload_url/file.download_url 等 action |

- **校验**：进程启动时加载配置并校验必填项（MQTT、MySQL、对象存储）；缺失或格式错误则启动失败并报错。
- **一致性**：多实例部署时各实例使用同一套配置（或通过环境变量区分仅连接串等），保证连接同一 Broker、同一 MySQL 与同一对象存储。

### 3.2 MQTT 网关层

- **连接**：服务端从**统一配置**读取 MQTT 连接信息，使用固定 ClientId（如 `mchat_gateway_1`）或带实例 ID 的 ClientId 连接 Broker，订阅 **`$share/gateway/mchat/msg/req/+/+`**（或非共享的 `mchat/msg/req/+/+`）。
...
| config.storage | 获取对象存储读写配置 | 校验当前员工身份 → 使用**统一配置中的对象存储鉴权信息**生成临时凭证或预签名策略，返回 endpoint、bucket、region、credentials 或 url_prefix、expires_at；**客户端据此直接上传/下载文件**，无需经服务端中转 |
...
| file.upload_url / file.download_url | 大文件单次临时 URL（可选） | 校验身份 → 使用**统一配置中的对象存储鉴权**调 S3 兼容 API 生成单次临时 URL → 返回 data 中的 url、expires_at 等；与 config.storage 可并存（配置用于客户端直连，本 action 用于按次链接） |
